...
export default async function RelationsPage() {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-4">
      <div>
        <h1 className="text-2xl font-bold uppercase tracking-wide text-foreground">Relations</h1>
      ...
      {relationships.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {relationships.map((rel) => {
            ...
            return (
              <div key={rel.id} className="panel flex items-center justify-between p-3 text-sm">
                <span className="text-foreground">
                  {from?.firstName} {from?.lastName}
                </span>
                <span className="border border-border-strong bg-surface-raised px-2 py-1 text-xs text-accent-strong">
